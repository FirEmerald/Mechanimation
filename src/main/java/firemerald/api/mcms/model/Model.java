...
public abstract class Model<M extends Model<M, T>, T extends Bone<T>> implements IModel<M, T>
{
	...
	{
		if (base instanceof RenderBone) ((RenderBone<?>) base).render(map, holder, defaultTexture);
	}
	...
	public void load(AbstractElement root)
	{
		...
		{
			base = this.makeNew(el.getString("name", "unnamed bone"), new Transformation(el), null);
			base.loadFromXML(el);
		}
		updateBonesList();
	}

...
public abstract class MultiModel<M extends MultiModel<M, T>, T extends Bone<T>> implements IModel<M, T>
{
	...
	@Override
	public void render(Map<String, Matrix4d> map, @Nullable Object holder, Runnable defaultTexture)
	{
		for (T base : this.base) if (base instanceof RenderBone<?>) ((RenderBone<?>) base).render(map, holder, defaultTexture);
	}
	...
	public void load(AbstractElement root)
	{
		...
		{
			T base = this.makeNew(el.getString("name", "unnamed bone"), new Transformation(el), null);
			base.loadFromXML(el);
			this.base.add(base);
		}
		updateBonesList();

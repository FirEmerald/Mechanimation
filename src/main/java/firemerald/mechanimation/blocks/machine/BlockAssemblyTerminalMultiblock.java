...
public class BlockAssemblyTerminalMultiblock<T extends BlockAssemblyTerminal> extends BlockContainer
{
	...
	public BlockAssemblyTerminalMultiblock(T parent)
	{
    ...
    {
		return this.getDefaultState().withProperty(DIRECTION, EnumFacing.VALUES[meta % 6]);
    }
	...
	{
		return new TileEntityAssemblyTerminalPart<>();
	}
    ...
    {
    	BlockPos oPos = pos.offset(state.getValue(DIRECTION));
    	IBlockState block = world.getBlockState(oPos);
    	if (block.getBlock() == this) return ((BlockAssemblyTerminalMultiblock<?>) block.getBlock()).getParentPos(world, oPos, block);
    	else if (block.getBlock() == this.parent) return oPos;
    	else return null;
    }
    ...
    public boolean isPowered(World world, BlockPos pos, IBlockState blockState)
    {
        if (world.isBlockPowered(pos)) return true;
    	{
    		...
    		if (newState.getBlock() instanceof BlockAssemblyTerminalMultiblock) //facing this
    		{
    			if (((BlockAssemblyTerminalMultiblock<?>) newState.getBlock()).isPowered(world, newPos, newState)) return true;
    		}
    	}
    	return false;
    }

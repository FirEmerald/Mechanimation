...
public abstract class TileEntityCatalyticReformer<T extends TileEntityCatalyticReformer<T>> extends TileEntityOrientedEnergyFluidItemMachineBase<T, ICatalyticReformerRecipe> implements IFluxReceiver
{
    public static final ResourceLocation CATALYTIC_REFORMER_GUI_TEXTURES = new ResourceLocation(MechanimationAPI.MOD_ID, "textures/gui/container/catalytic_reformer.png");
	...
	FACE_FLUID_INPUT = { EnumFace.TOP },
	FACE_PRIMARY_FLUID_OUTPUT = { EnumFace.LEFT },
	FACE_SECONDARY_FLUID_OUTPUT = { EnumFace.BACK };
	@SuppressWarnings("unchecked")
	public static final ItemInventoryTemplate<? extends TileEntityCatalyticReformer<?>> ITEM_TEMPLATE = ((ItemInventoryTemplateBuilder<? extends TileEntityCatalyticReformer<?>>) new ItemInventoryTemplateBuilder<>())
    		...
    		.build();
	@SuppressWarnings("unchecked")
	public static final FluidInventoryTemplate<? extends TileEntityCatalyticReformer<?>> FLUID_TEMPLATE = ((FluidInventoryTemplateBuilder<? extends TileEntityCatalyticReformer<?>>) new FluidInventoryTemplateBuilder<>())
			.addSlot(FLUID_INDEX_INPUT, TileEntityCatalyticReformer::getMaxFluid, TileEntityCatalyticReformer::getMaxFluidTransfer, machine -> CatalyticReformerRecipes::isCraftable, TileEntityCatalyticReformer::getMaxFluidTransfer, CommonFluidPredicates.accept(), machine -> FACE_FLUID_INPUT)
			.addSlot(FLUID_INDEX_PRIMARY_OUTPUT, TileEntityCatalyticReformer::getMaxFluid, machine -> 0, CommonFluidPredicates.deny(), TileEntityCatalyticReformer::getMaxFluidTransfer, CommonFluidPredicates.accept(), machine -> FACE_PRIMARY_FLUID_OUTPUT)
			.addSlot(FLUID_INDEX_SECONDARY_OUTPUT, TileEntityCatalyticReformer::getMaxFluid, machine -> 0, CommonFluidPredicates.deny(), TileEntityCatalyticReformer::getMaxFluidTransfer, CommonFluidPredicates.accept(), machine -> FACE_SECONDARY_FLUID_OUTPUT)
    		.build();
	@SuppressWarnings({ "unchecked"})
	public static final EnergyInventoryTemplate<? extends TileEntityCatalyticReformer<?>> ENERGY_TEMPLATE = ((EnergyInventoryTemplateBuilder<? extends TileEntityCatalyticReformer<?>>) new EnergyInventoryTemplateBuilder<>())
			.addSlot(ENERGY_INDEX_ENERGY, TileEntityCatalyticReformer::getMaxEnergy, TileEntityCatalyticReformer::getMaxEnergyTransfer, CommonEnergyPredicates.accept(), machine -> 0, CommonEnergyPredicates.deny(), machine -> EnumFace.values())
    		.build();
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static final OperationsTemplate<TileEntityCatalyticReformer> OPERATION_TEMPLATE = new OperationsTemplate<TileEntityCatalyticReformer>()
			.addOperation(CommonOperations.<TileEntityCatalyticReformer>energyFromItem(machine -> machine.ITEM_INDEX_BATTERY_INPUT, machine -> machine.ENERGY_INDEX_ENERGY_INPUT, TileEntityCatalyticReformer::getMaxEnergyTransferItem))
			.addOperation(CommonOperations.<TileEntityCatalyticReformer>fluidFromItem(machine -> machine.ITEM_INDEX_FLUID_INPUT, machine -> machine.FLUID_INDEX_FLUID_INPUT, TileEntityCatalyticReformer::getMaxFluidTransferItem))
			.addOperation(CommonOperations.<TileEntityCatalyticReformer>fluidToItem(machine -> machine.ITEM_INDEX_PRIMARY_FLUID_OUTPUT, machine -> machine.FLUID_INDEX_PRIMARY_FLUID_OUTPUT, TileEntityCatalyticReformer::getMaxFluidTransferItem))
			.addOperation(CommonOperations.<TileEntityCatalyticReformer>fluidToItem(machine -> machine.ITEM_INDEX_SECONDARY_FLUID_OUTPUT, machine -> machine.FLUID_INDEX_SECONDARY_FLUID_OUTPUT, TileEntityCatalyticReformer::getMaxFluidTransferItem));
	...
	public TileEntityCatalyticReformer()
    {
    	...
    	ITEM_INDEX_BATTERY_INPUT = this.itemInventory.getItemSlot(ITEM_TEMPLATE.getIndex(ENERGY_INDEX_ENERGY));
		...
		{
			renderDimsEnergyInput = new RenderInfo(8, 9, IGuiElements.ENERGY_BAR_PROGRESS); //TODO adjust
	...
	@Override
	public boolean canProcessRecipeThisTick(ICatalyticReformerRecipe recipe)
	{
	...
	@Override
	public void onUsageTick(ICatalyticReformerRecipe recipe)
	{
		progress += this.getMaxEnergyUsagePerTick();
	}

    @Override
	public ICatalyticReformerRecipe getValidRecipe()
    {
    	FluidOrGasStack fluidInput = FLUID_INDEX_FLUID_INPUT.getFluid();
    	if (fluidInput == null) return null;
    	ICatalyticReformerRecipe recipe = CatalyticReformerRecipes.getRecipe(fluidInput);
    	if (CatalyticReformerRecipes.isCraftable(recipe))
    	{
	...
	@Override
	public void processRecipe(ICatalyticReformerRecipe recipe)
	{
		FluidOrGasStack fluidInput = FLUID_INDEX_FLUID_INPUT.getFluid();
		FluidOrGasStack testStack = null;
		for (FluidOrGasStack fluid : recipe.getFluidInput())
    ...
    {
        return "mechanimation:catalytic_reformer";
    }
	...
	public int getField(int id)
    {
        ...
        case 1:
        	ICatalyticReformerRecipe recipe = this.getValidRecipe();
	...
	{
		container.addSlotToContainer(new SlotMachine<>((T) this, ITEM_INDEX_BATTERY_INPUT.getSlot(), 8, 53));
	...
	{
		return CATALYTIC_REFORMER_GUI_TEXTURES;
	}
	...
	{
		if (ConfigJEI.INSTANCE.catalyticReformerRecipes.val && adjY >= 25 && adjY < 31 && ((adjX >= 94 && adjX < 108) || (adjX >= 68 && adjX < 82))) return new String[] {RecipeCategoryCatalyticReformer.ID};
		else return null;
	}
	...
	{
		return "container.mechanimation.catalytic_reformer";
	}

package firemerald.mechanimation.tileentity.machine.merox_treater;
...
public abstract class TileEntityMeroxTreater<T extends TileEntityMeroxTreater<T>> extends TileEntityOrientedEnergyFluidItemMachineBase<T, IMeroxTreaterRecipe> implements IFluxReceiver
{
    public static final ResourceLocation MEROX_TREATER_GUI_TEXTURES = new ResourceLocation(MechanimationAPI.MOD_ID, "textures/gui/container/merox_treater.png");
	...
	FACE_WATER_OUTPUT = { EnumFace.FRONT };
	@SuppressWarnings("unchecked")
	public static final ItemInventoryTemplate<? extends TileEntityMeroxTreater<?>> ITEM_TEMPLATE = ((ItemInventoryTemplateBuilder<? extends TileEntityMeroxTreater<?>>) new ItemInventoryTemplateBuilder<>())
    		...
    		.build();
	@SuppressWarnings("unchecked")
	public static final FluidInventoryTemplate<? extends TileEntityMeroxTreater<?>> FLUID_TEMPLATE = ((FluidInventoryTemplateBuilder<? extends TileEntityMeroxTreater<?>>) new FluidInventoryTemplateBuilder<>())
			.addSlot(FLUID_INDEX_INPUT, TileEntityMeroxTreater::getMaxFluid, TileEntityMeroxTreater::getMaxFluidTransfer, machine -> MeroxTreaterRecipes::isCraftable, TileEntityMeroxTreater::getMaxFluidTransfer, CommonFluidPredicates.accept(), machine -> FACE_FLUID_INPUT)
			.addSlot(OXYGEN_INDEX_INPUT, TileEntityMeroxTreater::getMaxFluid, TileEntityMeroxTreater::getMaxFluidTransfer, machine -> stack -> stack.getContentsName().equals("oxygen"), TileEntityMeroxTreater::getMaxFluidTransfer, CommonFluidPredicates.accept(), machine -> FACE_OXYGEN_INPUT)
			.addSlot(FLUID_INDEX_OUTPUT, TileEntityMeroxTreater::getMaxFluid, machine -> 0, CommonFluidPredicates.deny(), TileEntityMeroxTreater::getMaxFluidTransfer, CommonFluidPredicates.accept(), machine -> FACE_FLUID_OUTPUT)
			.addSlot(WATER_INDEX_OUTPUT, TileEntityMeroxTreater::getMaxFluid, machine -> 0, CommonFluidPredicates.deny(), TileEntityMeroxTreater::getMaxFluidTransfer, CommonFluidPredicates.accept(), machine -> FACE_WATER_OUTPUT)
    		.build();
	@SuppressWarnings({ "unchecked"})
	public static final EnergyInventoryTemplate<? extends TileEntityMeroxTreater<?>> ENERGY_TEMPLATE = ((EnergyInventoryTemplateBuilder<? extends TileEntityMeroxTreater<?>>) new EnergyInventoryTemplateBuilder<>())
			.addSlot(ENERGY_INDEX_ENERGY, TileEntityMeroxTreater::getMaxEnergy, TileEntityMeroxTreater::getMaxEnergyTransfer, CommonEnergyPredicates.accept(), machine -> 0, CommonEnergyPredicates.deny(), machine -> EnumFace.values())
    		.build();
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static final OperationsTemplate<TileEntityMeroxTreater> OPERATION_TEMPLATE = new OperationsTemplate<TileEntityMeroxTreater>()
			.addOperation(CommonOperations.<TileEntityMeroxTreater>energyFromItem(machine -> machine.ITEM_INDEX_ENERGY_INPUT, machine -> machine.ENERGY_INDEX_ENERGY_INPUT, TileEntityMeroxTreater::getMaxEnergyTransferItem))
			.addOperation(CommonOperations.<TileEntityMeroxTreater>fluidFromItem(machine -> machine.ITEM_INDEX_FLUID_INPUT, machine -> machine.FLUID_INDEX_FLUID_INPUT, TileEntityMeroxTreater::getMaxFluidTransferItem))
			.addOperation(CommonOperations.<TileEntityMeroxTreater>fluidFromItem(machine -> machine.ITEM_INDEX_OXYGEN_INPUT, machine -> machine.FLUID_INDEX_OXYGEN_INPUT, TileEntityMeroxTreater::getMaxFluidTransferItem))
			.addOperation(CommonOperations.<TileEntityMeroxTreater>fluidToItem(machine -> machine.ITEM_INDEX_FLUID_OUTPUT, machine -> machine.FLUID_INDEX_FLUID_OUTPUT, TileEntityMeroxTreater::getMaxFluidTransferItem))
			.addOperation(CommonOperations.<TileEntityMeroxTreater>fluidToItem(machine -> machine.ITEM_INDEX_WATER_OUTPUT, machine -> machine.FLUID_INDEX_WATER_OUTPUT, TileEntityMeroxTreater::getMaxFluidTransferItem));
	...
	@Override
	public boolean canProcessRecipeThisTick(IMeroxTreaterRecipe recipe)
	{
	...
	@Override
	public void onUsageTick(IMeroxTreaterRecipe recipe)
	{
		this.progress += this.getMaxEnergyUsagePerTick();
	}

	@Override
	public void processRecipe(IMeroxTreaterRecipe recipe) //TODO onFluidSlotChanged
	{
		...
		FLUID_INDEX_WATER_OUTPUT.add(FluidOrGasStack.forFluid(new FluidStack(FluidRegistry.WATER, recipe.getRequiredOxygen() * 2)));
    ...
    @Override
	public IMeroxTreaterRecipe getValidRecipe()
    {
    	FluidOrGasStack fluidInput = FLUID_INDEX_FLUID_INPUT.getFluid();
    	if (fluidInput == null) return null;
    	IMeroxTreaterRecipe recipe = MeroxTreaterRecipes.getRecipe(fluidInput);
    	if (MeroxTreaterRecipes.isCraftable(recipe))
    	{
	...
	public int getField(int id)
    {
        ...
        case 1:
        	IMeroxTreaterRecipe recipe = this.getValidRecipe();
	...
	{
		return MEROX_TREATER_GUI_TEXTURES;
	}
	...
	{
		if (ConfigJEI.INSTANCE.meroxTreaterRecipes.val && adjX >= 81 && adjX < 94 && adjY >= 26 && adjY < 50) return new String[] {RecipeCategoryMeroxTreater.ID};
		else return null;
	}
	...
	{
		return "mechanimation:merox_treater";
	}

	@Override
	public String getDefaultName()
	{
		return "container.mechanimation.merox_treater";
	}

...
public class BasicMachineRecipeTransferHandler<T extends IItemMachine<T> & IGuiMachine<T>> implements IMachineRecipeTransferHandler<T>
{
	private final StackHelper stackHelper;
	private final IRecipeTransferHandlerHelper handlerHelper;
	private final IMachineRecipeTransferInfo<T> transferHelper;

	public BasicMachineRecipeTransferHandler(StackHelper stackHelper, IRecipeTransferHandlerHelper handlerHelper, IMachineRecipeTransferInfo<T> transferHelper)
	{
	...
	public IRecipeTransferError transferRecipe(ContainerMachine<T> container, IRecipeLayout recipeLayout, EntityPlayer player, boolean maxTransfer, boolean doTransfer)
	{
		...
		if (!transferHelper.canHandle(container)) return handlerHelper.createInternalError();
		Map<Integer, Slot> inventorySlots = new HashMap<>();
		for (Slot slot : transferHelper.getInventorySlots(container)) inventorySlots.put(slot.slotNumber, slot);
		Map<Integer, Slot> craftingSlots = new HashMap<>();
		for (Slot slot : transferHelper.getRecipeSlots(container)) craftingSlots.put(slot.slotNumber, slot);
		int inputCount = 0;
		IGuiItemStackGroup itemStackGroup = recipeLayout.getItemStacks();
		for (IGuiIngredient<ItemStack> ingredient : itemStackGroup.getGuiIngredients().values()) if (ingredient.isInput() && !ingredient.getAllIngredients().isEmpty()) inputCount++;
		if (inputCount > craftingSlots.size())
		{
			Log.get().error("Recipe Transfer helper {} does not work for machine {}", transferHelper.getClass(), container.machine.getClass());
		...
		for (Slot slot : craftingSlots.values())
		{
			...
			if (!stack.isEmpty())
			{
				...
				{
					Log.get().error("Recipe Transfer helper {} does not work for machine {}. Player can't move item out of Crafting Slot number {}", transferHelper.getClass(), container.machine.getClass(), slot.slotNumber);
					return handlerHelper.createInternalError();
				}
